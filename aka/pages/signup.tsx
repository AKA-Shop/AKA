...
const signup = () => {
    ...
    const signup=(body)=>{
        axios.post('http://localhost:8080/user/signUp',body).then(res=>window.location.href='/'
        ).catch(err=>alert(err)
        )
    } 
    const SignIn=(body)=>{
        axios.post('http://localhost:8080/user/login',body).then(res=>window.location.href='/'
        ).catch(err=>alert(err)
        )
    }
  return (
